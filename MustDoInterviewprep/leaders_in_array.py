...
def find_leaders_in_array(array_list):
	result_list = []
	element = 0
	for x in range(len(array_list)-1,-1,-1):
		if array_list[x] >= element:
			result_list.insert(0,array_list[x])
			element = array_list[x]
	return result_list
...
for i in range(test_cases):
	...
	result = find_leaders_in_array(array_element)
	for i in result:
		print(i, end = ' ')
	print('')
